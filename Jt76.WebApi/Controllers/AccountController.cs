namespace Jt76.WebApi.Controllers
{
	...
	public class AccountController : Controller
	{
		...
		public async Task<IActionResult> GetUserById(string id)
		{
			...
			User userVM = await GetUserHelper(id);
		...
		{
			List<Tuple<ApplicationUser, string[]>> usersAndRoles = await _accountManager.GetUsersAndRolesAsync(page, pageSize);

			List<User> usersVM = new List<User>();

			foreach (Tuple<ApplicationUser, string[]> item in usersAndRoles)
			{
				User userVM = Mapper.Map<User>(item.Item1);
		...
		public async Task<IActionResult> Register([FromBody] UserEdit user)
		{
			if (ModelState.IsValid)
			{
				if (user == null)
		...
		public async Task<IActionResult> GetRoleByName(string name)
		{
			...
			Role roleVM = await GetRoleHelper(name);
		...
		[HttpGet("permissions")]
		[Produces(typeof(List<Permission>))]
		[Authorize(AuthPolicies.ViewRolesPolicy)]
		...
		private async Task<User> GetUserHelper(string userId)
		{
			...
			User userVM = Mapper.Map<User>(userAndRoles.Item1);
		...
		private async Task<Role> GetRoleHelper(string roleName)
		{
